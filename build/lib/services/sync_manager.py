    ...
    async def run_sync(
        ...
        for idx, current_date in enumerate(trading_dates):
            ...
            if options.get("stocks") or options.get("etfs"):
                ...
                try:
                    ...
                    if e.response.status_code == 404:
                        logger.info(f"Bhavcopy 404 for {current_date.isoformat()} (Weekend/Holiday). Skipping.")
                    else:
            ...
            if options.get("indexes"):
                try:
                    ...
                    # Logs already written in service
                    pass
        ...
        if options.get("stocks"):
            ...
            try:
                ...
                master_isins = set(df_equity["ISIN NO"].astype(str).str.strip())
                for s in stocks:
        ...
        if options.get("market_cap"):
            ...
            if missing_shares_stocks:
                ...
                for idx, stock in enumerate(missing_shares_stocks):
                    ...
                    try:
                        quote = await self.client.fetch_stock_quote(stock.symbol)
