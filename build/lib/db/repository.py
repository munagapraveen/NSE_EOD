...
def bulk_upsert_adjusted_prices(session: Session, records: list[dict]) -> int:
    """
    Bulk upsert adjusted prices checking for duplicates.
    """
    if not records:
        return 0
    ...
    # Query existing records
    existing = session.execute(
        select(AdjustedPrice.security_id, AdjustedPrice.trade_date)
        .where(AdjustedPrice.security_id.in_(security_ids))
        .where(AdjustedPrice.trade_date.in_(trade_dates))
    ).all()
    existing_keys = {(row.security_id, row.trade_date) for row in existing}
    ...
    if new_records:
        session.bulk_insert_mappings(AdjustedPrice, new_records)
        session.commit()
