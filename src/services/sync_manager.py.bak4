...
from src.services.indicators import calculate_all_indicators, calculate_incremental_indicators_for_range
...
class SyncManager:
    ...
    def _detect_post_processing_gaps(self, session: Session) -> dict:
        """Detect raw prices missing adjusted prices or indicators from previous failed runs."""
        from sqlalchemy import func as sqlfunc, distinct

        # Count distinct (security_id, trade_date) pairs in each table
        raw_count = session.query(
            sqlfunc.count(distinct(sqlfunc.concat(RawPrice.security_id, '-', RawPrice.trade_date)))
        ).scalar() or 0

        adj_count = session.query(
            sqlfunc.count(distinct(sqlfunc.concat(AdjustedPrice.security_id, '-', AdjustedPrice.trade_date)))
        ).scalar() or 0

        ind_count = session.query(
            sqlfunc.count(distinct(sqlfunc.concat(Indicator.security_id, '-', Indicator.trade_date)))
        ).scalar() or 0
        ...
        return {
            ...
            "adj_count": adj_count,
            "ind_count": ind_count,
            "has_adj_gap": raw_count > adj_count,
            "has_ind_gap": adj_count > ind_count,
        }
    ...
    async def run_sync(
        ...
        # 5. FETCH SHARES OUTSTANDING (stocks only)
        if self._cancel_requested: return self._cancel_summary()
        if options.get("market_cap"):
            if progress_callback:
                progress_callback("SHARES_OUTSTANDING", 65.0, "Fetching outstanding shares from quote API (new/missing only)...")
            
            # Find active stocks that have NULL issued_shares
            ...
            if missing_shares_stocks:
                logger.info(f"Fetching issued shares count for {len(missing_shares_stocks)} stocks...")
                consecutive_failures = 0
                for idx, stock in enumerate(missing_shares_stocks):
                    if self._cancel_requested: return self._cancel_summary()
                    
                    # Update progress slightly
                    sub_pct = 65.0 + (idx / len(missing_shares_stocks)) * 10.0
                    if progress_callback:
                        progress_callback("SHARES_OUTSTANDING", sub_pct, f"Fetching shares for {stock.symbol} ({idx+1}/{len(missing_shares_stocks)})...")
                    
                    try:
                        quote = await self.client.fetch_stock_quote(stock.symbol, retries=0)
                        # Extract issuedSize from quote (structure: quote['marketDeptOrderBook']['tradeInfo']['issuedCap'] or similar)
                        # Safe extract:
                        ...
                        if issued_cap:
                            ...
                            session.commit()
                            logger.info(f"Updated shares outstanding for {stock.symbol}: {stock.issued_shares}")
                        consecutive_failures = 0  # Reset counter on success
                    except Exception as e:
                        logger.warning(f"Failed to fetch quote for {stock.symbol}: {e}")
                        consecutive_failures += 1
                        
                        if consecutive_failures >= 5:
                            logger.error("Too many consecutive quote fetch failures (likely rate-limited/blocked by NSE). Aborting shares outstanding fetch stage.")
                            break
                        # Keep going to prevent locking
                        await asyncio.sleep(1.0)

        # Determine if database already has price history and if the date range is small
        has_history = session.query(RawPrice).first() is not None
        is_incremental = has_history and (end_date - start_date).days <= 10
        logger.info(f"Sync type detection: has_history={has_history}, range_days={(end_date - start_date).days}, is_incremental={is_incremental}")

        # Detect gaps from previous failed runs and force global recalculation if needed
        if is_incremental:
            gaps = self._detect_post_processing_gaps(session)
            if gaps["has_adj_gap"] or gaps["has_ind_gap"]:
                logger.warning(
                    f"Detected post-processing gaps from a previous run! "
                    f"Raw={gaps['raw_count']}, Adj={gaps['adj_count']}, Ind={gaps['ind_count']}. "
                    f"Forcing global recalculation to fill gaps..."
                )
                is_incremental = False
            else:
                logger.info(f"No post-processing gaps detected (Raw={gaps['raw_count']}, Adj={gaps['adj_count']}, Ind={gaps['ind_count']})")

        # 6. SYNC CORPORATE ACTIONS
        if self._cancel_requested: return self._cancel_summary()
        if options.get("corporate_actions"):
            if progress_callback:
                progress_callback("CORPORATE_ACTIONS", 75.0, "Fetching corporate splits and bonuses...")
            try:
        ...
        # Close session and dispose engine to release Windows file locks before backup
        session.close()
