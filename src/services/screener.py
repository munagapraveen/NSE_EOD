...
def run_sharpe_screener(
    ...
    mcap_threshold_cr = mcap_filter_cr  # Database stores market cap in Crores directly
    ...
    logger.info("Target Dates Resolved:")
    ...
    for sec_id, grp in df_prices.groupby("security_id"):
        ...
        circuit_hits = 0
        bands = [5.0, 10.0, 20.0]
        tol = 0.15  # 0.15% tolerance to accommodate tick-size rounding (5 paise)
...
def _write_excel_sheet(ws, df, title, header_fill, alt_fill, target_date: date, long_months=6, short_months=3):
    ...
    title_cell = ws.cell(row=1, column=1, value=f"{title}  |  Screening Date: {target_date_str}  |  Exported: {today_str}")
...
def export_screener_to_excel(
    ...
    if df_filtered.empty:
        ...
        _write_excel_sheet(
            ws2,
            df_filtered,
            title="Filtered Watchlist  |  3M ROC > 20%  |  Close >= 75% of 52WH  |  Circuit Hits <= 10",
