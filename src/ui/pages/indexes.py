...
def get_indexes_list():
    """Fetch all Indexes with their latest close price."""
    session = SessionLocal()
    try:
        # Get latest trading date
        latest_date = session.query(func.max(RawPrice.trade_date)).scalar()
        
        # Select Indexes with outer joins to get latest price
        query = (
            select(
                Security.symbol,
...
def render():
    """Render the Indexes directory page."""
    indexes_data = get_indexes_list()
    
    with ui.column().classes("w-full gap-6 p-6"):
        # Page Title
        with ui.column().classes("gap-1"):
            ui.label("Indexes Directory").classes("text-3xl font-bold text-white")
            ui.label("List of all dynamically discovered official NSE Indexes and their latest closes.").classes("text-sm text-slate-400")
            
        # Stocks Table Component
        if indexes_data:
            create_stock_table(indexes_data)
        else:
            with ui.card().classes("glass-card w-full p-8 text-center items-center"):
                ui.icon("warning", size="lg").classes("text-amber-500 mb-2")
                ui.label("No Indexes Data Found").classes("text-lg font-bold text-white")
